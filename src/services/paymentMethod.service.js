...
const getAllData = async (query, employeeUnitId) => {
  const { page, limit, search, startDate, endDate, sortBy, sortOrder } = query;
  ...
  if (startDate && endDate) {
    filters.date = {
      gte: dayjs.tz(startDate, TIMEZONE).startOf("day").toDate(),
      lte: dayjs.tz(endDate, TIMEZONE).endOf("day").toDate(),
    };
  } else if (startDate) {
    filters.date = {
      gte: dayjs.tz(startDate, TIMEZONE).startOf("day").toDate(),
  ...
  // ✅ Filter berdasarkan unit employee
  filters.eventUnits = {
    some: {
      unitId: employeeUnitId,
    },
  };

  const orderBy = {};
  if (sortBy && ["name", "date", "createdAt"].includes(sortBy)) {
    orderBy[sortBy] =
      sortOrder && ["asc", "desc"].includes(sortOrder.toLowerCase())
        ? sortOrder.toLowerCase()
        : "asc";
  } else {
    orderBy["date"] = "desc"; // default sorting by date desc
  }
  ...
  return {
    results: events.map((event) =>
      eventResource(event, { summaryUnits: true })
    ),
    pagination: {
      total,
      page,
